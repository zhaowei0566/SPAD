...
StruPara.bdfname='naturalspace12by12.bdf';
% StruPara.pathfile='Z:\Paper\Stiffened Plate\StiffenedPlateCode\InputBDF';
StruPara.pathfile=[pwd filesep 'InputBDF'];
FEMPara=geometryinfoplate(StruPara);
...
%%
panel_point_1=[0,0];
panel_point_2=[Plate.length,0];
panel_point_3=[Plate.length,Plate.width];
panel_point_4=[0.0,Plate.width];
...
for stiffenerNumber=1:size(XINPUT,1)
    figure(200);hold on;
    plot(XXstiffener(stiffenerNumber,:),YYstiffener(stiffenerNumber,:),'b-','LineWidth',2);hold on;
end;
hold off;
