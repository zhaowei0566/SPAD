function [Kbeam,Kelem,StiffenerLength]=LaminatedStiffenerPerpendicularV1(FEM,Ds,Stiffener)
%% For stiffener lamina perpendicular to the reference plane
disp(['-------------Stiffener Material Stiffness-------------']);
FEM.typeplate='CBAR3';
...
for e=1:numberElements%% number of stiffener element
    ...
    for ee=1:size(GaussWeights,1) %% Gaussian Points number
        ...
        alpha=asin(naturalderivatives*XYZ(:,2)/detJ);
        ...
        zeronumber=[1,2,4,5];
        Dst=zeros(5,5);
        Dst(zeronumber,zeronumber)=Ds(zeronumber,zeronumber);
