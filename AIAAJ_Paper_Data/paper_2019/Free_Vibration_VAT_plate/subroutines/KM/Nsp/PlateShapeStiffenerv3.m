function [ShapeMatrixSP,ShapeMatrixElem]=PlateShapeStiffenerv3(FEM,Stiffener,TargetGaussian,TargetPlate)
...
%      disp(['-------------stiffener node No#' num2str(ee) '-------------']);
    ...
    TargetGaussianElm=TargetGaussian(ee,:); %% xi and eta
    plate=TargetPlate(ee);
    platenode=FEM.elementNodes(plate,:);
    xi=TargetGaussianElm(1);
    eta=TargetGaussianElm(2);
    ...
    ShapeMatrixElem(ee,platenode)=ShapeMatrixElem(ee,platenode)+  NshapeElement(ee,platenode);
