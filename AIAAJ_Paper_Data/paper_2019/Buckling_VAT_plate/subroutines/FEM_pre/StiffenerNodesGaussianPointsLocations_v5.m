function [TargetGaussian,TargetPlateElement,Residual]=StiffenerNodesGaussianPointsLocations_v5(FEM,Stiffener)
...
% %     right_eta_id=find(round(solution.eta)<=1&round(solution.eta)>=-1);
    ...
    right_xi_id=find(round(double(solution.xi),1)<=1&round(double(solution.xi),1)>=-1);
    right_eta_id=find(round(double(solution.eta),1)<=1&round(double(solution.eta),1)>=-1);
