function [stress_global,strain_global]=StressRecoveryPlate_VAT_Lagrange_NLV(FEM,Laminate,Mat,Stru,center,VAT)
...
for layer=1:Laminate.layer
    ...
    for elem=1:FEM.numberElements
        ...
        ShearStrain=zeros(2,size(GaussWeights,1));
         ...
         GaussLocations = GaussLocations(5,:); GaussWeights = 1;
...
% %             
            ...
            %             T1 = T0T1(layer,2);
            %
            %             theta(gauss_point_num) = VAT_fiber_ply_angle_1D(T0,T1,centerX,center,width);
            %
            %
            %
            %
            %         end
            %
            %          theta_ave  = sum(theta)/gauss_point_num;
            %
            %
            % theta =   theta_ave;
            ...
            %             % no bending curvature considered for in-plane stress
            %              strain0(:,gauss_point_num)=...
            %                [NormalStrain(1,gauss_point_num),...
            %                 NormalStrain(2,gauss_point_num), ...
            %                 NormalStrain(3,gauss_point_num)]';
            %
