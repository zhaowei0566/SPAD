...
for elem = 1:size(FEM.elementNodes)
    ...
    for layer = 1:size(T0T1,1)
        ...
        switch VAT.VAT_type
            ...
            case 'LV-Y'
                theta(layer) = VAT_fiber_ply_angle_1D(T0,T1,centerY,center(2),physical_length);
            case 'LV-XY'
                phi  = VAT.rigid_rotation_angle;
                theta(layer) = VAT_fiber_ply_angle_1D_rotate(T0,T1,centerX,centerY,center,physical_length,phi);
        end
...
%
% save NXX_SP_QI.txt NXX -ascii
% save NYY_SP_QI.txt NYY -ascii
% save NXY_SP_QI.txt NXY -ascii


% save NXX_SP_SSCC_SF.txt NXX -ascii
% save NYY_SP_SSCC_SF.txt NYY -ascii
% save NXY_SP_SSCC_SF.txt NXY -ascii
