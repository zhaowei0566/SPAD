function [stress_global,strain_global]=StressRecoveryPlate_VAT_center_average_v2_Y_constant(FEM,Laminate,Mat,Stru,T0T1,center,physical_length)
...
for layer=1:size(T0T1,1)
    ...
    for elem=1:FEM.numberElements
        ...
        ShearStrain=zeros(2,size(GaussWeights,1));
              ...
              GaussLocations = GaussLocations(5,:); GaussWeights = GaussWeights(5);
        ...
        for gauss_point_num=1:size(GaussWeights,1)
            ...
            centerY = sum(FEM.nodeCoordinates_label( NodeIndices,3))/length( NodeIndices);
