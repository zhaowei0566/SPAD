function [stress_global,strain_global]=StressRecoveryPlate_VAT_center_average_LV_constant_angle(FEM,Laminate,Mat,Stru,VAT)
...
for layer=1:size(T0T1,1)
    ...
    for elem=1:FEM.numberElements
        ...
        for gauss_point_num=1:size(GaussWeights,1)
            ...
            switch VAT.VAT_type
                case 'LV-X'
                    theta = VAT_fiber_ply_angle_1D(T0,T1,Element_center_X,center(1),physical_length);
                case 'LV-Y'
                    theta = VAT_fiber_ply_angle_1D(T0,T1,Element_center_Y,center(2),physical_length);
                case 'LV-XY'
                    phi  = VAT.rigid_rotation_angle;
                    theta = VAT_fiber_ply_angle_1D_rotate(T0,T1,Element_center_X,Element_center_Y,center,physical_length,phi);
            end
            
  
            
            [~,~,~,~,Qbar_K,~]=LaminatedComposite_VAT(Mat,Stru,Laminate,layer,theta);
